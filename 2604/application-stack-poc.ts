...
export class ApplicationStackPoc extends Stack {
    constructor(scope: Construct, id: string, props: ApplicationStackProps) {
        ...
        const executionRole = iam.Role.fromRoleArn(
            this,
            'ExecutionRole',
            envConfig.ECSService.ExecutionRoleArn,
            { mutable: false },
        );
        ...
        const cluster = ecs.Cluster.fromClusterAttributes(this, 'Cluster', {
            clusterName: envConfig.ECSService.ClusterName,
            vpc,
        });
        ...
        const cpu =
            ...
            isPrd() &&
            targetSubGroup === 'pluswa'
                ? '2048'
                : targetContainer === 'cmpf_splitter' && isNotDev()
                  ? '4096'
                  : '512';
        const memory =
            targetContainer === 'cmpf_service' &&
        ...
        const service = new ecs.FargateService(this, 'Service', {
            ...
            vpcSubnets: {
                subnets: envConfig.ECSService.Subnets.map((id, index) =>
                    ec2.Subnet.fromSubnetId(this, `Subnet${index}`, id),
                ),
            },
            securityGroups: envConfig.ECSService.SecurityGroups.map(
                (id, index) =>
        ...
        const listenerRule = new elb.ApplicationListenerRule(
            this,
            'ListenerRule',
            {
                listener,
                ...
                conditions: [
                    elb.ListenerCondition.pathPatterns([`${apiPath}*`]),
                ],
        ...
        const cfnListenerRule = listenerRule.node
            .defaultChild as elb.CfnListenerRule;
        cfnListenerRule.addPropertyDeletionOverride('Actions');

        // CloudWatch Logs メトリクスフィルター
        metricFilterProps.forEach((props, index) => {
            new logs.MetricFilter(this, `MetricFilter${index}`, {
                ...props,
                logGroup,
            });
        });
